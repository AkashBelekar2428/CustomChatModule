...
public extension ProgressHUD {
    ...
    class var isLoading: Bool {
        get { shared.isLoading }
        set { shared.isLoading = newValue }
    }
    
    class var spinnerAtTop: Bool {
        get { shared.spinnerAtTop }
        set { shared.spinnerAtTop = newValue }
    }
}
...
public class ProgressHUD: UIView {
    ...
    private var colorHUD		= UIColor.clear //UIColor.systemGray
	private var colorStatus		= UIColor.label
	private var colorAnimation	= UIColor.clear//UIColor.lightGray
	private var colorProgress	= UIColor.clear//UIColor.lightGray
    ...
    private var spinnerAtTop = false
	...
	private func setup(status: String? = nil, progress: CGFloat? = nil, animatedIcon: AnimatedIcon? = nil, staticImage: UIImage? = nil, hide: Bool, interaction: Bool) {
		...
		setupToolbar()
		setupLabel(status)
        self.accessibilityIdentifier = "Loader"
	...
	private func setupToolbar() {
		...
		if (toolbarHUD == nil) {
            ...
            toolbarHUD?.accessibilityIdentifier = "Toolbar"
	...
	@objc private func setupPosition(_ notification: Notification? = nil) {
		...
		var center = CGPoint(x: screen.size.width/2, y: (screen.size.height-heightKeyboard)/2)
        
        if self.spinnerAtTop {
            center = CGPoint(x: screen.size.width/2, y: 80)
            toolbarHUD?.barTintColor = UIColor.clear
            toolbarHUD?.setBackgroundImage(UIImage(), forToolbarPosition: .any, barMetrics: .default)

            // Remove the background view to make the toolbar completely transparent
            for view in toolbarHUD!.subviews {
